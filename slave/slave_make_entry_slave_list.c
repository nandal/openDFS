...
/*	myDFS_int	server_sockfd;
	...
	write(server_sockfd, &msg, sizeof(myDFS_msg));
	perror("hi is writen to socket");
	write(server_sockfd, slave_node_ptr, sizeof(myDFS_node));
	perror("node info is written to socket.");
	read(server_sockfd, &msg, sizeof(myDFS_msg));
